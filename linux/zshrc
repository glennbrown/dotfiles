# .zshrc
# Linux Specific zsh configuration file
#
...
zstyle :compinstall filename '/home/gbrown/.zshrc'
...
# Enabled Autosuggestions based on history
...
if [ -x $(command -v fzf) ]; then
	...
	# Fallback for older fzf versions that don't support --zsh
...
# Only run starship when available AND not on a text/serial console
if command -v starship >/dev/null 2>&1; then
  case "$(tty)" in
    /dev/tty[0-9]*|/dev/ttyS*|/dev/ttyAMA*|/dev/hvc*)
      # On a text/serial console → do nothing
      ;;
    *)
      # All other terminals (pts/ttyUSB/GUI terminals, SSH, etc.)
